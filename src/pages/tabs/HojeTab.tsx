...
export function HojeTab({
  employees, active, offToday, onVacation,
  coveredSts, uncovSts, stations, getEmployee, schedule,
  pieData, weekData, goToPracas, goToEquipe, goToEscala,
}: Props) {
    ...
    <div>
      ...
      <div className="ph-row-211">
        ...
        <div className="ph-card">
          ...
          {employees.length === 0 && (
            <div style={{ padding: '20px 18px', color: C.textLight, fontSize: 13, textAlign: 'center' }}>Nenhum funcionário cadastrado</div>
          )}
        ...
        <div className="ph-card ph-card-p" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center' }}>
          ...
          <div style={{ fontWeight: 700, fontSize: 14, color: C.text, marginBottom: 8 }}>
            {uncovSts.length > 0
              ? `${uncovSts.length} praça${uncovSts.length > 1 ? 's' : ''} sem cobertura`
              : stations.length === 0 ? 'Sem praças' : 'Tudo coberto!'}
          </div>
