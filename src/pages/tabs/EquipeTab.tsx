import { Calendar, Pencil, Trash2, Umbrella, UserCheck, Users, Briefcase, Plus } from 'lucide-react'
...
interface Props {
  employees: Employee[]
  schedule: Schedule
  onSetFolga: (id: number) => void
  onClearToday: (id: number) => void
...
export function EquipeTab({
  employees, schedule,
  onSetFolga, onClearToday, onEdit, onVacation, onDelete, onAdd, onManageRoles,
}: Props) {
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <button onClick={onManageRoles}
            style={{ ...addBtnSt, background: C.card, color: C.textMid, border: `1px solid ${C.border}` }}>
            <Briefcase size={14} strokeWidth={2} /> Cargos
          </button>
          <button onClick={onAdd} style={addBtnSt}>
            <Plus size={14} strokeWidth={2.5} /> Funcionário
          </button>
        </div>
      </div>
      ...
      {active.length > 0 && (
        <>
          ...
          <div className="ph-equipe-grid" style={{ marginBottom: 20 }}>
            {active.map(e => (
              <EmployeeCard key={e.id} emp={e} schedule={schedule}
                onSetFolga={onSetFolga} onClearToday={onClearToday}
                onEdit={onEdit} onVacation={onVacation} onDelete={onDelete} />
            ))}
          </div>
        </>
      )}
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            {status === 'vacation' && vacRange && (
              <div style={{ fontSize: 11, color: C.info, marginTop: 3, display: 'flex', alignItems: 'center', gap: 3 }}>
                ...
                {fmtDate(vacRange.start)} → {fmtDate(vacRange.end)}
              </div>
            )}
          </div>
          {/* Edit + Delete */}
          <div style={{ display: 'flex', gap: 5 }}>
            <button onClick={() => onEdit(emp)} style={iconEditBtnSt} title="Editar">
              <Pencil size={13} strokeWidth={2} />
            </button>
            <button onClick={() => setConfirmDel(true)}
              style={{ ...iconEditBtnSt, background: C.dangerLight, borderColor: C.danger + '40' }} title="Remover">
        ...
        {confirmDel && (
          <div style={{ background: C.dangerLight, border: `1px solid ${C.danger}35`, borderRadius: 11, padding: '10px 12px', marginBottom: 12, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}>
            <span style={{ fontSize: 12, color: C.danger, fontWeight: 600 }}>
              Remover "{emp.name.split(' ')[0]}"?
            </span>
            <div style={{ display: 'flex', gap: 6 }}>
              <button onClick={() => setConfirmDel(false)}
                style={{ background: C.bg, border: `1px solid ${C.border}`, borderRadius: 8, padding: '4px 9px', cursor: 'pointer', fontSize: 12, color: C.textMid, fontFamily: 'DM Sans,sans-serif' }}>
                Não
              </button>
              <button onClick={() => onDelete(emp.id)}
                style={{ background: C.danger, border: 'none', borderRadius: 8, padding: '4px 9px', cursor: 'pointer', fontSize: 12, color: '#fff', fontWeight: 700, fontFamily: 'DM Sans,sans-serif' }}>
                Sim
              </button>
            </div>
          </div>
